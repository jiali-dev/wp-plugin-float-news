jQuery(function ($) {
  ...
  $(document).on("click", ".jialifn-bookmark-button", function (e) {
    e.preventDefault();

    const el = $(this);
    const wrapper = $(this).closest(".jialifn-bookmark");
    ...
    const countEl = el.find(".jialifn-bookmark-count");
    let count = parseInt(countEl.text().replace(/\D/g, "")) || 0;
    const oldCount = count;
    const isActive = el.hasClass("jialifn-bookmark-button-active");

    // ===== OPTIMISTIC UPDATE =====
    el.toggleClass("jialifn-bookmark-button-active", !isActive);
    el.find(".jialifn-icon")
    ...
    $.ajax({
      ...
      data: {
        nonce: jialifn_ajax.nonce,
        post_id,
        action: "jialifn_bookmark_toggle_ajax",
      },
      error: function (xhr) {
        const msg =
          xhr?.responseJSON?.message ||
          jialifn_translate_handler.error_occurred;
